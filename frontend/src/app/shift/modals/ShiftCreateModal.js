...
const ShiftCreateModal = ({ onClose, onSuccess }) => {
  ...
  const validate = () => {
    ...
    else if (form.name.length > 50) e.name = 'Tên ca tối đa 50 ký tự!';
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <button type="submit" className="btn text-white px-4 fw-bold"
              style={{ background: 'linear-gradient(135deg, #0ea5e9, #3b82f6)', border: 'none', borderRadius: '10px', fontSize: '0.9rem' }}
              disabled={loading}>
              {loading
