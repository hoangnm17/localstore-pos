...
const ShiftCreateModal = ({ onClose, onSuccess }) => {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) {
      e.name = 'Tên ca không được để trống!';
    } else if (form.name.trim().length < 3) {
      e.name = 'Tên ca phải có ít nhất 3 ký tự!';
    } else if (form.name.trim().length > 50) {
      e.name = 'Tên ca tối đa 50 ký tự!';
    }

    if (!form.startTime) {
      e.startTime = 'Chọn giờ bắt đầu!';
    }

    if (!form.endTime) {
      e.endTime = 'Chọn giờ kết thúc!';
    }

    if (form.startTime && form.endTime) {
      const shiftDuration = getDuration(form.startTime, form.endTime);

      if (shiftDuration === 0) {
        e.endTime = 'Giờ kết thúc không được trùng giờ bắt đầu!';
      } else if (shiftDuration < 5) {
        e.endTime = 'Ca làm việc tối thiểu 5 phút!';
      } else if (shiftDuration > 600) {
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      ...
      if (name === 'startTime' && value) {
        ...
        updated.checkInStart = toTime(startMin - 2);
        updated.checkInEnd = toTime(startMin + 2);
      }

      if (name === 'endTime' && value && !prev.checkOutDeadline) {
        const endMin = toMin(value);
        updated.checkOutDeadline = toTime(endMin + 2);
      }
