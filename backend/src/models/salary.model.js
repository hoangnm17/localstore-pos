...
module.exports.getSalaryReport = async (month, year, staffId = null, roleName = null) => {
    ...
    const result = await request.query(`
        SELECT 
            ...
            -- Với role hourly (Cashier) → đếm ngày thực có ca
            CASE
                WHEN r.name IN ('Manager', 'Warehouse') 
                THEN ${totalDaysInMonth}
                ELSE COUNT(DISTINCT CASE WHEN ws.id IS NOT NULL THEN CAST(ws.workDate AS DATE) END)
            END AS workingDays,
        ...
        LEFT JOIN WorkSchedules ws
            ...
            -- Không lấy ca "Ca HC" auto-created cho Manager để tránh tính giờ sai
            AND NOT (r.name IN ('Manager', 'Warehouse') AND ws.snapshotShiftName LIKE N'Ca HC%')
            
        LEFT JOIN Shifts sh ON ws.shiftId = sh.id
        CROSS APPLY (
            SELECT 
                effStart = CASE WHEN ws.workDate <= CAST(DATEADD(hour, 7, GETUTCDATE())
                 AS DATE) AND ws.snapshotStartTime IS NOT NULL THEN ws.snapshotStartTime 
                 ELSE ISNULL(sh.startTime, ws.snapshotStartTime)
                  END,
                effEnd = CASE WHEN ws.workDate <= CAST(DATEADD(hour, 7, GETUTCDATE()) 
                AS DATE) AND ws.snapshotEndTime IS NOT NULL THEN ws.snapshotEndTime 
    ...
    return result.recordset.map(row => {
        ...
        return {
            ...
            note: isMonthlyRole ? 'Lương cố định theo tháng' : '',
        };
    });
};
